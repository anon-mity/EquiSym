...
class ShapeNetH5Loader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.num_pts is not None and len(point_cloud) > self.num_pts:
            sampled_indices = np.random.choice(len(point_cloud), self.num_pts, replace=False)
...
if __name__ == "__main__":
    ...
    n_points = 1024
    batch_size = 128
    data_set = ShapeNetH5Loader(data_path=dataset_path, mode='train', category='plane', num_pts=1024)
    ...
    for id, data in enumerate(loader, 0):
        ...
        point_norm = point_norm.cuda()
        output_dir = '/home/hanbing/paper_code/vnn/visualizer/shapenet_plane'
        if id ==0:
            D = point_norm  # B,1024,3
        else:
            D = torch.cat((D, point_norm), dim=0)
    ...
    for i in range(B):
        ...
        for j in range(B):
            ...
            pcj = pcj.unsqueeze(0)
            if i != j:
                cd, _ = chamfer_distance(pci,pcj)
                cd_i += cd
        cd_imean = cd_i.item()/(B-1)
        cd_sum += cd_imean
    cd_mean = cd_sum / B
    print(cd_mean)